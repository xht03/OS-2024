...
public:
    template <typename... Args> void add(const Key& key, Args&&... args)
    {
    ...
    template <typename... Args> void try_add(const Key& key, Args&&... args)
    {
        std::unique_lock lock(mutex);
        map.try_emplace(key, std::forward<Args>(args)...); // args其实为空
    }
